...
import { createError, getQuery, readBody, sendStream, setHeader } from 'h3';
...
export async function handleShareStream(event, options: ResolveOptions) {
  ...
  if (shareLink.hasPassword) {
    ...
    const ok = bcrypt.compareSync(password, shareLink.passwordHash);
    if (!ok) {
...
async function parseBody(event) {
  return (await readBody<{ password?: string }>(event).catch(() => ({}))) ?? {};
}

function parseQuery(event) {
  const query = getQuery(event);
  return { password: typeof query.password === 'string' ? query.password : undefined };
}
